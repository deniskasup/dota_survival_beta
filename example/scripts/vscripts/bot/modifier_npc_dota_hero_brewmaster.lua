modifier_npc_dota_hero_brewmaster = class({})
...
function modifier_npc_dota_hero_brewmaster:DeclareFunctions()
	local funcs = 
	{
...
--当buff创建
function  modifier_npc_dota_hero_brewmaster:OnCreated(keys)
	...
	boss_abilitys = {"brewmaster_thunder_clap","brewmaster_cinder_brew","brewmaster_drunken_brawler","sven_gods_strength","sven_great_cleave","ursa_overpower","weaver_geminate_attack","elder_titan_natural_order","huskar_berserkers_blood","tusk_walrus_punch"}
	boss_items = {"item_refresher","item_nullifier","item_black_king_bar","item_satanic","item_harpoon","item_monkey_king_bar","item_ex_machina","item_assault"}
	...
	boss:CastAbilityImmediately(boss:FindAbilityByName("brewmaster_drunken_brawler"),0)
	boss:CastAbilityImmediately(boss:FindAbilityByName("brewmaster_drunken_brawler"),0)
	...
	--禁用自动施法
	no_cast = {brewmaster_drunken_brawler=1,mars_bulwark=1,bristleback_bristleback=1,troll_warlord_berserkers_rage=1,bounty_hunter_jinada=1,tusk_walrus_punch=1,weaver_geminate_attack=1}
	...
	self:GetParent():SetContextThink("bot_auto_cast",function()
		...
		if(casting_ability:IsCooldownReady() and self:GetParent():GetHealthPercent()<20)then
			self:GetParent():CastAbilityImmediately(casting_ability,0)
		end
		--否决
		...
		--鱼叉
		local casting_ability = self:GetCaster():FindItemInInventory("item_harpoon")
		if(bad_hero[1] and casting_ability:IsCooldownReady())then
			pcall(function()self:GetParent():SetCursorCastTarget(bad_hero[1])end)
			self:GetParent():CastAbilityImmediately(casting_ability,0)
		end
...
--不可驱散
function  modifier_npc_dota_hero_brewmaster:IsPurgable()return false end
--永久
function  modifier_npc_dota_hero_brewmaster:IsPermanent()return true end
