modifier_npc_dota_hero_pangolier = class({})
...
function modifier_npc_dota_hero_pangolier:DeclareFunctions()
	local funcs = 
	{
...
--当buff创建
function  modifier_npc_dota_hero_pangolier:OnCreated(keys)
	...
	boss_abilitys = {"pangolier_shield_crash","pangolier_lucky_shot","pangolier_gyroshell2","bloodseeker_rupture","batrider_flaming_lasso","medusa_stone_gaze","primal_beast_trample","sven_gods_strength","dazzle_weave","tiny_grow","enchantress_untouchable","crystal_maiden_freezing_field","crystal_maiden_frostbite"}
	boss_items = {"item_heart","item_trident","item_bloodstone","item_shivas_guard","item_skadi","item_overwhelming_blink","item_lotus_orb","item_assault"}
	...
	self:GetParent():SetContextThink("bot_attack",function()
		targets = FindUnitsInRadius(self:GetParent():GetTeamNumber(),self:GetParent():GetCenter(),nil,46000,DOTA_UNIT_TARGET_TEAM_ENEMY,DOTA_UNIT_TARGET_HERO,DOTA_UNIT_TARGET_FLAG_NONE,FIND_CLOSEST,false)
		...
		if targets[AttackTarget[1]] then
			...
			order_table = {OrderType=1,UnitIndex=self:GetParent():GetEntityIndex(),Position=self.target:GetOrigin()}
	...
	--禁用自动施法
	no_cast = {crystal_maiden_frostbite=1,bristleback_bristleback=1,brewmaster_drunken_brawler=1,mars_bulwark=1,troll_warlord_berserkers_rage=1,bounty_hunter_jinada=1,tusk_walrus_punch=1,weaver_geminate_attack=1}
	...
	self:GetParent():SetContextThink("bot_auto_cast",function()
		...
		for i=0,20 do
			...
			if ability and self:GetParent():IsAlive() and not ability:IsHidden() and not ability:IsPassive() and not ability:IsToggle() and ability:GetLevel()>0 and ability:IsCooldownReady() and not no_cast[ability:GetName()]and not self:GetParent():IsChanneling()then
				...
				--如果是滚滚大
				if ability:GetName() == "pangolier_gyroshell2" then
					self:GetParent():CastAbilityImmediately(ability,0)
				end
			end
		end
...
--不可驱散
function  modifier_npc_dota_hero_pangolier:IsPurgable()return false end
--永久
function  modifier_npc_dota_hero_pangolier:IsPermanent()return true end
